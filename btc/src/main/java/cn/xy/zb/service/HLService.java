	...
	public void work(){
		...
		if("r".equals(on)){
			try {
				...
				if(total.compareTo("2000000000")<0){
					if(total.compareTo(k2)!=0) {//如果k2和k1不相等
						memcachedClient.set("k1", k2);
						memcachedClient.set("k2", total);
					}
					memcachedClient.set("on", "f");//打开开关,然后往前移1位
				}else if(total.compareTo("2100000000")>=0 && total.compareTo("2200000000")<0) {
				...
				}else if(total.compareTo("2200000000")>=0 && total.compareTo("2300000000")<0){
					if(total.compareTo(k2)>0) {
						memcachedClient.set("on", "t");//打开开关,然后往前移1位
						memcachedClient.set("k1", k2);
						memcachedClient.set("k2", total);
